...
colmat <- function(nquantiles = 5, upperleft = "#0096EB", upperright = "#820050", 
                   bottomleft = "#c74c00", bottomright = "#edb600",
...
col.matrix<-colmat(nquantiles=5, upperleft = "#c90028", upperright = "#fadd02", bottomleft = "lightblue", bottomright = "forestgreen", xlab="AC penetration (quintile)", ylab="CDDs (quintile)")

ggsave(plot = last_plot(),
       filename = "biv_legend_cdd.png",
       device = "png", path = getwd(), height = 5, width = 5,
       dpi = 320, scale=1, bg='transparent')

####

r1_view <- rast("F:/.shortcut-targets-by-id/1JhN0qxmpnYQDoWQdBhnYKzbRCVGH_WXE/6-Projections/rscripts/global_spline/output_data/ncdf/ac_penetration_ssp1_ssp2_ssp3_ssp5_2010_2050.nc")
r1_view <- r1_view[[6]]
r1_view <- mean(r1_view)
r1_view <- raster(r1_view)

#
cdds <- list.files(pattern="cdd24", full.names = T, path="F:/.shortcut-targets-by-id/13znqeVDfPULc4J_lQLbyW_Kmfa03o63F/3-Research/GIACOMO/AGE_EXPOSURE")
c1 <- rast(cdds[5])
c1 <- mean(c1)
c1 <- raster(c1)
...
r1_view <- projectRaster(r1_view, c1)
  
rhist <- stack(r1_view, c1)
names(rhist) <- c("AC (%)", "CDDs")
...
sf_use_s2(T)
regions = filter(regions, NAME_0 !="Antarctica" & !is.na(NAME_0))
regions$REGION_n <- as.numeric(regions$NAME_0)
regions_r <- fasterize::fasterize(regions, r1_view[[1]], field="REGION_n")
regions <- dplyr::select(regions, NAME_0, REGION_n)
...
r = raster::crop(rhist, extent(regions_globe))
...
bivmap <- bivariate.map(rasterx = r[["AC...."]], rastery = r[["CDDs"]],
...
map <- ggplot(data=bivMapDF) +
  ggtitle("AC penetration and cooling degree days")+
...
ggsave(plot = map,
       filename = paste0("map_biv_cdd_ac.pdf"),
       device = "pdf", path = getwd(),
       dpi = 320, scale=2, height = 3)
