...
# positive indicators
pos_indic <- c("green_blue_infra_share", "ac", "radio_ownership_pctg", "tv_ownership_pctg", "cellphone_ownership_pctg", "computer_ownership_pctg", "multiple_information_means_ownership_pctg", "fridge_ownership_pctg", "primary_education_rate_women", "primary_education_rate_men", "mean_ac_reg")
...
cities_norm <- dplyr::select(cities_norm, -c(1:5))

for (i in 1:nrow(weights)){
cities_norm[,weights$indic[i]] <- ifelse(rep(weights$indic[i] %in% neg_indic, nrow(cities_norm)), normalize_variable(cities_norm[,weights$indic[i]]), -(normalize_variable(cities_norm[,weights$indic[i]])))
}
...
cities_norm <- bind_cols(cities[,c(1:5)], cities_norm)
...
cities_subset <- c("Bangkok", "Milan", "Kinshasa", "Recife")
